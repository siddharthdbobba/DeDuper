...
struct GroqReviewer {
    ...
    func review(items: [PhotoItem], scores: [Double]) async throws -> AIReviewResult {
        guard let apiKey = KeychainHelper.retrieve(key: "groq_api_key"), !apiKey.isEmpty else {
            throw ReviewerError.noAPIKey
        }

        // Groq decommissioned the llama-3.2-*-vision-preview family in early 2025;
        // requests to those names return HTTP 400 model_decommissioned. Transparently
        // upgrade saved settings so existing users don't have to know about it.
        let defaultModel = "meta-llama/llama-4-scout-17b-16e-instruct"
        let deprecatedModels: Set<String> = [
            "llama-3.2-11b-vision-preview",
            "llama-3.2-90b-vision-preview",
        ]
        let savedModel = UserDefaults.standard.string(forKey: "groqModel")
        let model: String
        if let saved = savedModel, !saved.isEmpty, !deprecatedModels.contains(saved) {
            model = saved
        } else {
            model = defaultModel
        }
        ...
        if let http = response as? HTTPURLResponse, !(200...299).contains(http.statusCode) {
            throw ReviewerError.httpError(http.statusCode, Self.extractAPIError(from: data))
        }
        ...
        return AIReviewResult(winnerIndex: winnerIndex, reason: reason, provider: .groq)
    }

    /// Pulls the human-readable error message out of an OpenAI-shaped error envelope
    /// (`{"error":{"message":"...","code":"..."}}`). Falls back to the raw body, truncated.
