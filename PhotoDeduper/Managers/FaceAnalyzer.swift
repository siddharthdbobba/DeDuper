...
struct FaceAnalyzer {
    ...
    ///
    /// `VNImageRequestHandler.perform` is *synchronous* and parks its caller on a
    /// semaphore until Vision has capacity. Running it directly inside this
    /// `async` function blocked a Swift-concurrency cooperative thread; with a
    /// group of photos scored at once, every thread in the fixed-width pool ended
    /// up parked here and the whole scan deadlocked (the "stuck at N%" stall —
    /// the watchdog timer could not run either, because running it also needs a
    /// cooperative thread). Hand the blocking call to a real Dispatch queue so the
    /// calling task merely suspends. See `BlockingWork`.
    static func analyze(_ image: CGImage) async -> Result {
        await BlockingWork.run { performAnalysis(image) }
    }

    /// The synchronous Vision work. Never call this from an `async` context
    /// directly — go through `analyze(_:)` so it lands off the cooperative pool.
    private static func performAnalysis(_ image: CGImage) -> Result {
